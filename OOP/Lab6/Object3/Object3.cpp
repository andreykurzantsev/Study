...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		GetTextFromClipboard(hWnd, bufferText, sizeof(bufferText)); // Берем текст из буфера
	}
	break;
	case WM_COPYDATA:
	{
	...
	case WM_PAINT:
	{
		...
		if (bufferText != tempBufferForMatrixString) {
			...
			buffer = new int[tempBufferForMatrixString.size()];
			int i = 0;
			string num;
